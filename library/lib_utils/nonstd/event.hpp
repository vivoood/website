...
public:
    ...
    template <class C>
    void disconnect(C * const object_ptr, void(C::* const method_ptr)(Args...)) noexcept
    {
    ...
    template <class C>
    void disconnect(C * const object_ptr, void(C::* const method_ptr)(Args...) const) noexcept
    {
        slot_type slot(object_ptr, method_ptr);
        _slots.erase(std::remove_if(std::begin(_slots), std::end(_slots),
            [&slot](const slot_type& other) { return slot == other; }
    ...
    template <class C>
    void disconnect(C & object, void(C::* const method_ptr)(Args...))  noexcept
    {
    ...
    template <class C>
    void disconnect(C const & object, void(C::* const method_ptr)(Args...) const) noexcept
    {
        slot_type slot(object, method_ptr);
        _slots.erase(std::remove_if(std::begin(_slots), std::end(_slots),
            [&slot](const slot_type& other) { return slot == other; }
    ...
    /// \param args The arguments to emit to the slots connected to the event
...
private:
    ...
    /// The slots connected to the event
    slot_array _slots;
};
}
#endif // EVENT_HPP
