...
class delegate<R(A...)>
{
    ...
    struct manager
    {
        constexpr manager(get_pointer_type getter, clone_type cloner, compare_type comparer, destroy_type destructor)
...
struct hash<nonstd::delegate<R(A...)>>
{
    size_t operator()(nonstd::delegate<R(A...)> const& d) const noexcept
    {
